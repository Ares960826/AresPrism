...
async fn accept_callback(listener: TcpListener) -> Result<(String, String), String> {
    ...
    let html = r#"<!DOCTYPE html><html><body style="font-family:system-ui;text-align:center;padding:60px"><h2>Connected to Zotero!</h2><p style="color:#666">You can close this tab and return to AresPrism.</p><script>setTimeout(()=>window.close(),1500)</script></body></html>"#;
    let response = format!(
...
pub async fn zotero_start_oauth(
    ...
    let authorize_url = format!(
        "{}?oauth_token={}&name=AresPrism&library_access=1&notes_access=0&write_access=0&all_groups=read",
        AUTHORIZE_URL, token
    );

...
describe("tauri.conf.json CSP configuration", () => {
  ...
  it("uses a bundle identity distinct from upstream ClaudePrism", () => {
    expect(conf.identifier).toBe("dev.ares.prism");
    expect(conf.identifier).not.toBe("com.claude-prism.desktop");
    expect(conf.productName).toBe("AresPrism");

...
- 关掉编辑器里的独立主文件时，右侧对应的 PDF 标签一起关掉，避免预览标签堆起来
- Agent：本机 CLI（登录/OAuth）和 API 密钥拆开。密钥只进 Claude Code；Codex / Grok / Kimi 仍用各自 CLI 登录

## 0.10.2 — 2026-09-19

- 启动后检查 GitHub 更新；设置 → Updates、左下角刷新按钮可手动检查并一键安装
